...
export function HeroSection({ isEmbedded = false }: { isEmbedded?: boolean }) {
  // Choose between Link component or anchor based on whether this is embedded
  const LinkComponent = ({ href, className, children }: { 
    ...
    if (isEmbedded) {
      // In embedded mode, use <a> tags with absolute URLs and target="_parent"
      const absoluteUrl = `https://stack.hidajat.me${href}`;
      return (
        <a href={absoluteUrl} target="_parent" className={className}>
          {children}
        </a>
      );
    } else {
      // In regular mode, use Next.js Link component
      return (
